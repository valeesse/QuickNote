...
fn open_popup(app: &tauri::AppHandle, label: &str) {
    if let Some(window) = app.get_webview_window(label) {
        if let Ok(Some(monitor)) = window.primary_monitor() {
            ...
            let x = (screen.width as f64 / scale) - win_w - 20.0;
            let y = (screen.height as f64 / scale) - win_h - 60.0;
    ...
    tauri::Builder::default()
        ...
        .setup(|app| {
            ...
            #[cfg(not(any(target_os = "android", target_os = "ios")))]
            {
                ...
                app.global_shortcut().register(shortcut_n).ok();
                app.global_shortcut().register(shortcut_c).ok();
                app.global_shortcut().register(shortcut_q).ok();
